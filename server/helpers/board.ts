...
export function getPieceById(id: PieceId, unplayedPieces: Piece[], board: IBoard): Piece {
  return (unplayedPieces.find((p) => p.id === id) || boardPiecesAsList(board).find((p) => p.id === id))!;
}

export function getTopPieceAtPos(position: BoardPosition, board: IBoard): Piece | undefined {
...
export function getBoardPositionFromKey(key: string): BoardPosition {
  ...
  if (vals.length !== 2) {
    throw new Error(`The provided key ${key} could not be parsed into a BoardPosition`);
  }
  return { x: vals[0], y: vals[1] };
}

export function getSurroundingPositions(positions: BoardPosition[]): BoardPosition[] {
...
export function getSurroundingPieces(position: BoardPosition, board: IBoard): Piece[] {
...
export function isPieceSurrounded(piece: Piece, board: IBoard): boolean {
  ...
  return getSurroundingPieces(piece.position, board).length === NUM_SIDES_OF_PIECE;
}

export function getFreelyMovableSpaces(position: BoardPosition, board: IBoard): BoardPosition[] {
 ...
 */
export function canFreelyMoveAcrossStacks(source: Piece, blockers: Piece[], target?: Piece): boolean {
  if (blockers.length < 2) {
    return true;
  }
  const minBlockerStack = Math.min(...blockers.map((b) => b.stack!));
  const maxSourceTargetStack = Math.max(source.stack! - 1, (target && target.stack!) || 0);
...
export function getFreelyClimbablePieces(piece: Piece, board: IBoard): Piece[] {
  ...
  return surroundingPieces.filter((n) => {
    const overlap = getOverlappingNeighbors(surroundingPositions, n.position!, board);
    return canFreelyMoveAcrossStacks(piece, overlap, n);
  });
}

export function getFreelyDroppableSpaces(piece: Piece, board: IBoard): BoardPosition[] {
  ...
  return surroundingPositions.filter(
    (t) =>
      ...
      canFreelyMoveAcrossStacks(piece, getOverlappingNeighbors(surroundingPositions, t, board))
  );
}
...
export function getOverlappingNeighbors(
  ...
  const surroundingPositions = Array.isArray(source) ? source : getSurroundingPositions([source]);
  ...
  return compact(
    getSurroundingPositions([target])
      .filter((n) => surroundingPositions.some((p) => p.x === n.x && p.y === n.y))
